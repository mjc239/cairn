...
def render(o: Outline, decls: dict[str, FormalDecl], title: str, style: Style, detail_note: str) -> str:
    ...
    for e in o.order:
        ...
        if e.startswith("S:"):
            ...
            lines.append(f"- **{kind}** `{v}`" + (" *(proof deferred)*" if deferred else ""))
            if d.type_pp:
                lines.append(f"  `{_short(d.type_pp)}`")
            if d.doc:
                lines.append(f"  — {_short(d.doc, 300)}")
        else:
            uses = sorted(node_of(p) for p in o.graph.predecessors(e) if node_of(p) != v)
            helpers = [h for h in o.absorbed.get(v, []) if ".Tactic." not in f".{h}."]  # hide tactic internals
            note = []
            if pos[e] - pos[S(v)] > 1:
