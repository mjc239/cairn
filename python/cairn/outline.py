...
# Classes whose instances state no mathematics: decidability (classically every proposition is decidable) and
# printing / hashing / boolean-equality code. Everything else, coercions and `Nonempty`/`Inhabited` included, can
# carry content a reader needs (how a notation reads, that a set is nonempty) and stays eligible.
_BOILERPLATE_CLASSES = {"Decidable", "DecidablePred", "DecidableRel", "DecidableEq", "Repr", "ToString", "Hashable",
                        "BEq"}


def is_boilerplate_instance(d: FormalDecl) -> bool:
    """An auto-named instance (``instDecidablePred…``) of a class with no mathematical content: decidability or
    printing/hashing code. Such instances are never presented as results. All other instances stay eligible,
    including coercions and ``Nonempty``/``Inhabited``; blueprints name instances of mathematical classes
    (``instModule``, ``instIsZLatticeE8Lattice``).
    """
...
def render(o: Outline, decls: dict[str, FormalDecl], title: str, style: Style, detail_note: str,
    ...
    from .prose import full_statement_of, helpers_of, open_namespaces, statement_of, uses_of
    ...
    lines = [f"# {title}", "",
             ...
             f"The Lean line is a short form (explicit hypotheses and assumptions with content); the complete "
             f"statement, with every implicit argument and instance assumption, is under *Full Lean statement*. "
             f"Namespaces {', '.join(nss)} are open.*"]
    if prose:
    ...
    for e in o.order:
        ...
        if e.startswith("S:"):
            ...
            if entry.get("statement"):
                ...
                if statement_of(d, nss):
                    lines.append(f"  `{_oneline(statement_of(d, nss))}`")
                if d.doc:
                    lines.append(f"  — {_oneline(d.doc)}")
            full = _oneline(full_statement_of(d, nss))
            if full and full != _oneline(statement_of(d, nss)):
                lines.append(f"  <details><summary>Full Lean statement</summary><code>{html.escape(full)}</code>"
                             "</details>")
        else:
            uses = uses_of(o, v)
            ...
            if uses:
                note.append("uses " + ", ".join(f"`{u}`" for u in uses))
            if helpers:
