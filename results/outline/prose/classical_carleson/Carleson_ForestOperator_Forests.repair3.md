...
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $f, g$ ($X \to \mathbb{C}$) is not stated
...
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $g$ ($X \to \mathbb{C}$) is not stated
...
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $f$ ($X \to \mathbb{C}$) is not stated
...
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $g_1, g_2$ ($X \to \mathbb{C}$) is not stated; $u_1,u_2$ not stated to be tiles in $\mathfrak{P}(X)$
...
Previous English: (Lemma 7.4.4) Let $X$ be a metric space and assume the standing assumptions of the proof: the `ProofData` bundle for the data $a \in \mathbb{N}$, $q \in \mathbb{R}$, $K : X \times X \to \mathbb{C}$, $\sigma_1, \sigma_2 : X \to \mathbb{Z}$ and $F, G \subseteq X$ (making $X$ a doubling metric measure space with the kernel $K$), together with a tile structure (`TileStructure`) for $Q$ with the default parameters $D = $ `defaultD a`, $\kappa = $ `defaultκ a`, $S = $ `defaultS X` and $o = $ `cancelPt X`. Under these assumptions, let $t$ be an $n$-forest, $u_1, u_2 \in t$ with $u_1 \ne u_2$, and let $g_1, g_2$ be bounded with compact support. Then $$\Big|\int_X T^*_{\mathfrak{T}(u_1)} g_1(x)\, \overline{T^*_{\mathfrak{T}(u_2)} g_2(x)}\,dx\Big| \le C_{7.4.4}(a,n)\, \big\|\mathbf{1}_{\mathcal{I}(u_1)\cap\mathcal{I}(u_2)} \cdot \mathrm{adjointBoundaryOperator}_t(u_1, g_1)\big\|_{L^2}\, \big\|\mathbf{1}_{\mathcal{I}(u_1)\cap\mathcal{I}(u_2)} \cdot \mathrm{adjointBoundaryOperator}_t(u_2, g_2)\big\|_{L^2}.$$
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $g_1, g_2$ ($X \to \mathbb{C}$) is not stated; the notation $T^*_{\mathfrak{T}(u)}$ is used but never introduced in this entry (it should be adjointCarlesonSum), and $C_{7.4.4}$ is not identified
...
Previous English: (The $g$ side of Proposition 2.0.4) Let $X$ be a metric space and assume the standing assumptions of the proof: the `ProofData` bundle for the data $a \in \mathbb{N}$, $q \in \mathbb{R}$, $K : X \times X \to \mathbb{C}$, $\sigma_1, \sigma_2 : X \to \mathbb{Z}$ and $F, G \subseteq X$ (making $X$ a doubling metric measure space with the kernel $K$), together with a tile structure (`TileStructure`) for $Q$ with the default parameters $D = $ `defaultD a`, $\kappa = $ `defaultκ a`, $S = $ `defaultS X` and $o = $ `cancelPt X`. Under these assumptions, let $t$ be an $n$-forest, $f$ measurable with $\|f(x)\| \le \mathbf{1}_F(x)$ and $g$ measurable with $\|g(x)\| \le \mathbf{1}_G(x)$ for all $x$. Then $$\Big|\int_X \overline{g(x)} \sum_{u \in t} \mathrm{carlesonSum}(\mathfrak{T}(u), f)(x)\,dx\Big| \le G_{2.0.4}(a,n)\, \|f\|_{L^2}\, \|g\|_{L^2}.$$
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $f, g$ ($X \to \mathbb{C}$) is not stated; $G_{2.0.4}$ not identified as `Forest.G2_0_4` in this entry
...
Checker's issue: the type of $n$ (a natural number) is not stated ('an $n$-forest'); the codomain of $f$ ($X \to \mathbb{C}$) is not stated; $\mu$ (the volume measure) is used without being introduced
