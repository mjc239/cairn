...
Constructor (every field with its type): `{X : Type u_1} → {a : ℕ} → {q : ℝ} → {K : X → X → ℂ} → {σ₁ σ₂ : X → ℤ} → {F G : Set X} → [inst : PseudoMetricSpace X] → [inst_1 : ProofData a q K σ₁ σ₂ F G] → (k : ℤ) → (hk : -↑(defaultS X (F := F) (G := G)) ≤ k) → k ≤ ↑(defaultS X (F := F) (G := G)) → (y : ↑(Yk X (F := F) (G := G) k)) → I3 (F := F) (G := G) hk y ⊆ I3 (F := F) (G := G) ⋯ (⟨cancelPt X, ⋯⟩ : { x : X // x ∈ Yk X (F := F) (G := G) ↑(defaultS X (F := F) (G := G)) }) → 𝓓 X (F := F) (G := G)`
...
#### `Encodable.linearOrder` (def)
Lean: `{α : Type u_2} → Encodable α → LinearOrder α`
Definition: `fun {α : Type u_2} (i : Encodable α) => LinearOrder.lift' Encodable.encode ⋯`

#### `Yk_encodable` (def)
Lean: `(X : Type u_1) → {a : ℕ} → {q : ℝ} → {K : X → X → ℂ} → {σ₁ σ₂ : X → ℤ} → {F G : Set X} → [inst : PseudoMetricSpace X] → [inst_1 : ProofData a q K σ₁ σ₂ F G] → (k : ℤ) → Encodable ↑(Yk X (F := F) (G := G) k)`
Definition: `fun (X : Type u_1) {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [PseudoMetricSpace X] [ProofData a q K σ₁ σ₂ F G] (k : ℤ) => Set.Countable.toEncodable (s := Yk X (F := F) (G := G) k) ⋯`
...
#### `C𝓩` (def)
Lean: `ℝ`
Docstring: The constant appearing in 4.2.2 (3 / 10).
Definition: `(3 / 10 : ℝ)`
...
Definition: `fun (X : Type u_1) {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [PseudoMetricSpace X] [ProofData a q K σ₁ σ₂ F G] => { Grid := 𝓓 X (F := F) (G := G), fintype_Grid := Fintype.ofFinite (𝓓 X (F := F) (G := G)), coeGrid := fun (z : 𝓓 X (F := F) (G := G)) => z.coe (F := F) (G := G), s := fun (z : 𝓓 X (F := F) (G := G)) => z.k (F := F) (G := G), c := fun (z : 𝓓 X (F := F) (G := G)) => ↑(z.y (F := F) (G := G)), inj := ⋯, range_s_subset := ⋯, topCube := max_𝓓 X (F := F) (G := G), s_topCube := ⋯, c_topCube := ⋯, subset_topCube := ⋯, Grid_subset_biUnion := ⋯, fundamental_dyadic' := ⋯, ball_subset_Grid := ⋯, Grid_subset_ball := ⋯, small_boundary := ⋯, coeGrid_measurable := ⋯ }`
...
Definition: `fun {X : Type u_1} {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [PseudoMetricSpace X] [ProofData a q K σ₁ σ₂ F G] [inst_2 : GridStructure X (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] => { toGridStructure := inst_2, 𝔓 := (I : Grid X) × ↥(𝓩 (F := F) (G := G) I), fintype_𝔓 := Sigma.instFintype (κ := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)), 𝓘 := fun (p : (I : Grid X) × ↥(𝓩 (F := F) (G := G) I)) => p.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)), surjective_𝓘 := ⋯, 𝒬 := fun (p : (I : Grid X) × ↥(𝓩 (F := F) (G := G) I)) => ↑(p.snd (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I))), range_𝒬 := ⋯ }`
...
Definition: `fun {X : Type u_1} {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [PseudoMetricSpace X] [ProofData a q K σ₁ σ₂ F G] [GridStructure X (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] => WellFounded.Nat.fix (motive := fun (p : 𝔓 X) => Set (@Θ _ X DenselyNormedField.toNormedField UniformSpace.toTopologicalSpace _)) (fun (x : 𝔓 X) => x.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)).opSize) fun (p : 𝔓 X) (a_1 : (y : 𝔓 X) → InvImage (fun (x1 x2 : ℕ) => x1 < x2) (fun (x : 𝔓 X) => x.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)).opSize) y p → Set (@Θ _ X DenselyNormedField.toNormedField UniformSpace.toTopologicalSpace _)) => if h : IsMax (p.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I))) then Construction.Ω₁ (F := F) (G := G) p else have this := ⋯; ball_{𝔠 p, ↑(defaultD a) ^ 𝔰 p / (4 : ℝ)} (𝒬 p) Construction.CΩ ∪ ⋃ (z : @Θ _ X DenselyNormedField.toNormedField UniformSpace.toTopologicalSpace _), ⋃ (hz : z ∈ ↑(𝓩 (F := F) (G := G) (p.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)).succ (F := F) (G := G))) ∩ Construction.Ω₁ (F := F) (G := G) p), a_1 (Sigma.mk (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)) (p.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)).succ (F := F) (G := G)) (⟨z, ⋯⟩ : ↥(𝓩 (F := F) (G := G) (p.fst (β := fun (I : Grid X) => ↥(𝓩 (F := F) (G := G) I)).succ (F := F) (G := G))))) ⋯`
...
Definition: `fun (X : Type u_1) {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [PseudoMetricSpace X] [ProofData a q K σ₁ σ₂ F G] [GridStructure X (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] => { toPreTileStructure := tileData_existence, Ω := Construction.Ω (F := F) (G := G), biUnion_Ω := ⋯, disjoint_Ω := ⋯, relative_fundamental_dyadic := ⋯, cball_subset := ⋯, subset_cball := ⋯ }`
