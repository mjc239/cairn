...
Checker's issue: The symbol Q (in `TileStructure Q ...`) is never introduced; it is the function X -> Theta(X) supplied by ProofData. 'The tree T(u)' is not tied to the forest t (t.𝔗 u).
...
Checker's issue: The symbol Q (in `TileStructure Q ...`) is never introduced; it is the function X -> Theta(X) supplied by ProofData.
...
Checker's issue: The symbol Q (in `TileStructure Q ...`) is never introduced; it is the function X -> Theta(X) supplied by ProofData. The tree notation T(u) (t.𝔗 u, the tree of u in the forest t) is not introduced/linked to t. The cube map I(.) (𝓘) is also not introduced.
...
Checker's issue: The symbol Q (in `TileStructure Q ...`) is never introduced; it is the function X -> Theta(X) supplied by ProofData. The tree notation T(u) (t.𝔗 u, the tree of u in the forest t) is not introduced/linked to t.
