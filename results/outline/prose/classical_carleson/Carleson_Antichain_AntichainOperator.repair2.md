...
Previous English: For a set of tiles $\mathfrak{A} \subseteq \mathfrak{P}(X)$, a tile $p$ and a function $g : X \to \mathbb{C}$, we define the quantity $h(p) = $ `dach 𝔄 p g` $\in [0, \infty]$ used in the proof of Lemma 6.1.4.
Checker's issue: The definition's English omits the ProofData and TileStructure standing assumptions the signature requires, and its gloss ('used in the proof of Lemma 6.1.4') is not attributed to the docstring.
