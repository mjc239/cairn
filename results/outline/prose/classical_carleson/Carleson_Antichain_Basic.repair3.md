...
Docstring: Constant appearing in Lemma 6.1.3. Has value `2 ^ (103 * a ^ 3)` in the blueprint.
Definition: `fun (a : ℕ) (q : NNReal) => (2 : NNReal) ^ ((𝕔 + (3 : ℕ)) * a ^ (3 : ℕ)) * (q - (1 : NNReal))⁻¹`
...
#### `Lemma6_1_3.p` (def)
Lean: `(X : Type u_1) → {a : ℕ} → {q : ℝ} → {K : X → X → ℂ} → {σ₁ σ₂ : X → ℤ} → {F G : Set X} → [inst : MetricSpace X] → [ProofData a q K σ₁ σ₂ F G] → ℝ`
Docstring: Exponent used for the application of Hölder's inequality in the proof of Lemma 6.1.3.
Definition: `fun (X : Type u_1) {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [MetricSpace X] [ProofData a q K σ₁ σ₂ F G] => ((3 / 2 : ℝ) - (Lemma6_1_3.qt X (F := F) (G := G))⁻¹)⁻¹`
...
Docstring: The `p` maximal function used in the proof.
Definition: `fun {X : Type u_1} {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [MetricSpace X] [ProofData a q K σ₁ σ₂ F G] [TileStructure (Q (F := F) (G := G)) (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] (𝔄 : Set (𝔓 X)) (p : ℝ) => maximalFunction volume 𝔄 𝔠 (fun (𝔭 : 𝔓 X) => (8 : ℝ) * HPow.hPow (α := ℝ) (↑(defaultD a)) (𝔰 𝔭)) ↑p.toNNReal`
...
#### `Lemma6_1_3.qt` (def)
Lean: `(X : Type u_1) → {a : ℕ} → {q : ℝ} → {K : X → X → ℂ} → {σ₁ σ₂ : X → ℤ} → {F G : Set X} → [inst : MetricSpace X] → [ProofData a q K σ₁ σ₂ F G] → ℝ`
Docstring: $\tilde{q}$ from definition 6.1.10, as a real number.
Definition: `fun (X : Type u_1) {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [MetricSpace X] [ProofData a q K σ₁ σ₂ F G] => (2 : ℝ) * ↑(nnq X (F := F) (G := G)) / (↑(nnq X (F := F) (G := G)) + (1 : ℝ))`

#### `Lemma6_1_3.𝓜` (def)
Lean: `{X : Type u_1} → {a : ℕ} → {q : ℝ} → {K : X → X → ℂ} → {σ₁ σ₂ : X → ℤ} → {F G : Set X} → [inst : MetricSpace X] → [inst_1 : ProofData a q K σ₁ σ₂ F G] → [inst_2 : TileStructure (Q (F := F) (G := G)) (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] → Set (𝔓 X) → (X → ℂ) → X → ENNReal`
Docstring: The maximal function used in the proof of Lemma 6.1.3
Definition: `fun {X : Type u_1} {a : ℕ} {q : ℝ} {K : X → X → ℂ} {σ₁ σ₂ : X → ℤ} {F G : Set X} [MetricSpace X] [ProofData a q K σ₁ σ₂ F G] [TileStructure (Q (F := F) (G := G)) (defaultD a) (defaultκ a) (defaultS X (F := F) (G := G)) (cancelPt X)] (𝔄 : Set (𝔓 X)) => maximalFunction volume 𝔄 𝔠 (fun (𝔭 : 𝔓 X) => (8 : ℝ) * HPow.hPow (α := ℝ) (↑(defaultD a)) (𝔰 𝔭)) (1 : ℝ)`
...
Checker's issue: The letter \mathfrak{c} means both the project constant 𝕔 (in D = 2^{\mathfrak{c} a^2}) and the tile centre 𝔠 (\mathfrak{c}(\mathfrak{p}), which is used in B_p). Use a different symbol for one of them. The content otherwise matches the Lean.
