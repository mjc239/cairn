...
Previous English: Let $f,g$ be bounded with compact support and let $\mathfrak C$ be a set of tiles. Then $$\int_X\overline{g(x)}\,T_{\mathfrak C}f(x)\,dx=\int_X\overline{T^*_{\mathfrak C}g(x)}\,f(x)\,dx,$$ i.e. $T^*_{\mathfrak C}$ is the adjoint of $T_{\mathfrak C}$.
