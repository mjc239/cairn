...
Previous English: Under the standing assumptions (a metric space $X$ with the `ProofData` bundle for parameters $a \in \mathbb{N}$, $q \in \mathbb{R}$, a complex-valued kernel $K$, integer-valued functions $\sigma_1,\sigma_2$ and sets $F, G \subseteq X$, and the tile structure on $X$ with the default parameters), for a natural number $n$, an $n$-forest $t$ on $X$ and $j \in \mathbb{N}$, $\mathrm{rowDecomp}_{\mathfrak{U}}(t, j)$ is a set of tiles in $\mathfrak{P}(X)$.
Checker's issue: Definition body is shown (recursive choice, via WellFounded.Nat.fix, of a maximal element of rowDecomp_zornset applied to t.𝔘 minus the earlier rows, i.e. a maximal set of tiles of t.𝔘 not in previous rows with pairwise disjoint cubes 𝓘 u and cubes maximal among those), but the English only says it is 'a set of tiles'; it does not describe what is defined.
...
Checker's issue: Definition body is shown (the n-row whose forest has 𝔘 = t.rowDecomp_𝔘 j and the same 𝔗 as t), but the English only gives the type and paraphrases the docstring; it does not describe the object by what it defines.
