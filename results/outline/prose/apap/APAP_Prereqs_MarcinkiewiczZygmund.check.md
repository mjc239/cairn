...
Lean (full): `∀ {ι : Type u_1} {A : Finset ι} {m n : ℕ}, m ≠ (0 : ℕ) → ∀ (f : ι → ℝ), (∀ (i : Fin n), ∑ a ∈ Fintype.piFinset fun x => A, f (a i) = (0 : ℝ)) → ∑ a ∈ Fintype.piFinset fun x => A, (∑ i, f (a i)) ^ ((2 : ℕ) * m) ≤ ((4 : ℝ) * ↑m) ^ m * ↑n ^ (m - (1 : ℕ)) * ∑ a ∈ Fintype.piFinset fun x => A, ∑ i, f (a i) ^ ((2 : ℕ) * m)`
...
Lean (full): `∀ {ι : Type u_1} {A : Finset ι} {m n : ℕ} {𝕜 : Type u_2} [inst : RCLike 𝕜], m ≠ (0 : ℕ) → ∀ (f : ι → 𝕜), (∀ (i : Fin n), ∑ a ∈ Fintype.piFinset fun x => A, f (a i) = (0 : 𝕜)) → ∑ a ∈ Fintype.piFinset fun x => A, ‖∑ i, f (a i)‖ ^ ((2 : ℕ) * m) ≤ ((8 : ℝ) * ↑m) ^ m * ↑n ^ (m - (1 : ℕ)) * ∑ a ∈ Fintype.piFinset fun x => A, ∑ i, ‖f (a i)‖ ^ ((2 : ℕ) * m)`
